...
const getAllLeaves = async (req, res) => {
  try {
    ...
    const { companyId, empId, currentUserId, status, viewType, id, from, startDate, endDate } = req.query;
    ...
    if (startDate && endDate) {
      ...
      console.log('INFO: Filtering leaves by date range:', { startDate, endDate });
    }
    ...
    if (normalizedType === 'me') {
      if (from === "teamleave") {
        ...
        if (currentUserId) {
          const currentUser = await EmployeeModel
            .findById(currentUserId)
            .select('employmentInformation.lineManager');
          ...
          if (lineManagerId) {
            ...
            console.log('INFO: TeamLeave(team) - filtering leaves by peers under same line manager');
          } else {
            // Fallback: if current user has no lineManager configured, show own leaves only
            filters.empId = currentUserId;
            console.log('WARN: TeamLeave(team) - current user has no lineManager; falling back to own leaves');
          }
        }
      } else {
        if (empId) {
          // When empId is provided, show only that employee's leaves (no approval logic)
          filters.empId = empId;
          console.log('INFO: Filtering leaves for specific employee');
        } else if (currentUserId) {
          // Default: user's own leaves + pending approvals
          useOrQuery = true;
          orConditions.push({ empId: currentUserId });
          orConditions.push({ 'currentApprovers.approverId': userObjectId, status: 'pending' });
          console.log('INFO: Filtering own leaves and pending approvals');
        }
      }
    } else if (normalizedType === 'myteam') {
      // Show team members' leaves (and optionally manager's own leaves)
      if (currentUserId) {
        const teamMembers = await EmployeeModel.find({
          companyId: cleanCompanyId,
          'employmentInformation.status': 'Active',
        ...
        filters.empId = { $in: teamIds };
        console.log('INFO: Filtering team member leaves');
      }
    } else if (normalizedType === 'myfunction') {
      // Show function members' leaves (same department)
      if (currentUserId) {
        ...
        if (userFunctions.length > 0) {
          ...
          filters.empId = { $in: functionIds };
          console.log('INFO: Filtering function member leaves');
        } else {
          filters.empId = currentUserId;
        }
      }
    } else if (normalizedType === 'mycompany') {
      // Company-wide leaves already scoped by companyId
      console.log('INFO: Filtering company-wide leaves');
    } else {
      ...
      if (viewType === "pending-approvals" && currentUserId) {
        ...
        console.log('INFO: Manager view - showing pending approvals only');
      } else if (viewType === "all-leaves" && currentUserId) {
        console.log('INFO: Admin view - showing all company leaves');
      } else if (currentUserId) {
        ...
        if (currentUser) {
          ...
          orConditions.push({ empId: currentUserId });
          orConditions.push({ 'currentApprovers.approverId': userObjectId, status: 'pending' });
          console.log('INFO: Employee access - showing own leaves and pending approvals');
    ...
    // Status filter (only if not using OR query or if specified)
    ...
    if (search) {
      ...
      if (useOrQuery) {
        ...
        delete filters.$or; // Remove the simple $or since we're using $and now
      } else {
        filters.$or = searchConditions;
      }
    }

    // Status filter (works with plain filters, $or, or $and from search + OR)
